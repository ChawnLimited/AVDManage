...
Function JoinEntraID
{
	try {
		...
		if ($MDMJoin -eq "Y") {Set-Service -Name dmwappushservice -StartupType Manual -ErrorAction SilentlyContinue;Set-Service -Name dcsvc -StartupType Manual -ErrorAction SilentlyContinue;$proc=Start-Process dsregcmd -ArgumentList "/AzureSecureVMJoin /debug /MdmId '0000000a-0000-0000-c000-000000000000'" -Passthru -Wait -RedirectStandardOutput AVD-EntraJoin.log;} else {$proc=Start-Process dsregcmd -ArgumentList "/AzureSecureVMJoin /debug" -Passthru -Wait -RedirectStandardOutput AVD-EntraJoin.log;}
