...
# Logfile is created in C:\Windows\AVD-Update\Update-VM-<date>.log
...
New-Item -Path C:\Windows\AVD-Update -ItemType Directory -Force
$Logfile = "C:\Windows\AVD-Update\Update-VM-$(get-date -f yyyy-MM-dd-HHmmss).log"
$WinUpLog= "C:\Windows\AVD-Update\WSUS-$(get-date -f yyyy-MM-dd-HHmmss).log"
...
try	{
	...
	$dotNETVer=(Get-ChildItem 'HKLM:\SOFTWARE\Microsoft\NET Framework Setup\NDP' -Recurse | Get-ItemProperty -Name version -EA 0 | Where { $_.PSChildName -Match '^(?!S)\p{L}'} | Select version)[0].version
	Logit "Microrost .Net Framework Version : $dotNETVer"
	}
catch	{}

# Update Root Certificate -----------------------------------------------------------------|
Function UpdateRootCerts{
	Logit "Updating Root Certificates"
	try	{
...
# remove expired certs
		...
		foreach ($c in $oldcerts) {Remove-Item -Path $c.PSPath -Force -ErrorAction SilentlyContinue}
		$rc=(Get-ChildItem -Path Cert:\LocalMachine\Root\).count
...
# Update Microsoft Edge - requires access to go.microsoft.com -----------------------------------------------------------------|
Function UpdateEdge {
	Logit "Updating Microsoft Edge"
	try{
...
# Update Google Chrome - requires access to dl.google.com -----------------------------------------------------------------|
Function UpdateChrome {
	Logit "Updating Google Chrome"
	try{
...
# Disable Chrome Updaters
		...
		else {Logit ("Chrome is not installed")}
	}
...
Function UpdateVisC {
	Logit "Updating Visual C Redistributables"
	try{
...
Function UpdateOffice{
	...
	try{	
		if (get-item -path "$env:ProgramFiles\Microsoft Office") 
		{
		Get-Service -Name ClickToRunSvc | Set-service -startuptype Automatic
...
# Disable Office Updates
		...
		else {Logit ("Office is not installed")}
	}
...
Function UpdateOneDrive {
	...
	try{
		...
		else {Logit ("OneDrive is not installed")}
	}
...
Function UpdateTeams {
	try	{
		...
		else {Logit ("Teams is not installed")}
	}
...
Function UpdateFSLogix {
	...
	try	{
		...
		Logit "FSlogix Version: $FSVER"
		Logit "Add Local Administrators to FS Logix Exclude Groups"
# Add Local Administrators to FS Logix Exclude Groups
		...
		else {Logit ("FSLogix is not installed")}
	}
	Catch	{}
}
...
# install PSWindowsUPdate
	...
	catch	{Logit "Failed to install PSWindowsUpdate"}

	try {
# Windows Update - requires access to windowsupdate.microsoft.com + others
# Enable Services
	Logit "Starting Windows Update"
...
# install Windows Updates but not the MRT KB890830
	Get-WindowsUpdate -MicrosoftUpdate -Install -AcceptAll -IgnoreReboot -UpdateType Software -NotKBArticleID KB890830 | Out-File $WinUpLog -force
...
# Disable Windows Maintenance Tasks
	...
	catch	{Logit "Windows Update failed"}
}


# update defender -----------------------------------------------------------------|
Function UpdateDefender {
	Logit "Updating Windows Defender"
	try{
...
# Start the updates -----------------------------------------------------------------|
	if ($ROOTCERTS) {UpdateRootCerts}
	if ($EDGE) {UpdateEdge}
	if ($CHROME) {UpdateChrome}
	if ($VISC) {UpdateVisC}
	if ($O365) {UpdateOffice}
	if ($ONEDRIVE) {UpdateOneDrive}
	if ($TEAMS) {UpdateTeams}
	if ($FSLOGIX) {UpdateFSLogix}
	if ($DEFENDER) {UpdateDefender}
	if ($WINDOWS) {UpdateWindows}
