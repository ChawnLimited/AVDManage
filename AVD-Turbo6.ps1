...
# AVD-Turbo6.ps1
# Version 6.0
# Enables joining MDM / Intune
# Updated RDS Agent and Bootloader download URLs 
# Rename the VM (if created from a Specialized Image), optionally Join VM to Active Directory or Entra ID (+ Intune), and optionally install AVD Agents and join AVD Host Pool - For Specialized and Generalized Images, RIP AVDJoin
...
#### Parameters
  ...
  param(
	...
	[String]$MDMJoin= "",				# Y|N Join Entra ID
	...
	[String]$AVDLogons="",				# Set the AVD Logs mode - True|False
...
Function JoinEntraID
{
	try {
		...
		if ($MDMJoin -eq "Y") {Set-Service -Name dmwappushservice -StartupType Manual -ErrorAction SilentlyContinue;Set-Service -Name dcsvc -StartupType Manual -ErrorAction SilentlyContinue;$proc=Start-Process dsregcmd -ArgumentList "/AzureSecureVMJoin /debug /MdmId 0000000a-0000-0000-c000-000000000000" -Passthru -Wait -RedirectStandardOutput AVD-EntraMDMJoin.log;} else {$proc=Start-Process dsregcmd -ArgumentList "/AzureSecureVMJoin /debug" -Passthru -Wait -RedirectStandardOutput AVD-EntraJoin.log;}
...
%{
	try {
		LogWrite ("Check Defender is not currently updating")
		do {start-sleep -Seconds 1} until ((get-process -name MpCmdRun -ErrorAction silentlycontinue).count -eq 0)
		LogWrite ("Defender is not currently updating")
	}
	catch{}
}
...
# Disable AVD Logons if required
%{
	try {
		if ($AVDLogons -eq "False") {
		logwrite ('Disable AVD Logons for: ' + $hostname + ' Hostpool: ' + $HostPool)
		$i=0
		$GETHostUri="https://management.azure.com/subscriptions/$subId/resourceGroups/$RG/providers/Microsoft.DesktopVirtualization/hostPools/$hostPool/sessionHosts/$HostName/?api-version=2024-04-03"
		do {start-sleep -seconds 2;$i++;try {$SessionHost=Invoke-RestMethod -Uri $GETHostURI -Method Get -Headers $Headers} catch{}} until ($SessionHost.name.count -eq 1 -or $i -eq 30)
			...
			Invoke-RestMethod -Uri $HostURI -Method Patch -Headers $Headers -Body $body
			logwrite ('AVD Logons disabled for: ' + $hostname + ' Hostpool: ' + $HostPool);$ErrorActionPreference="Continue"
			}
	}
    catch {Logwrite("1001 Failed to Disable AVD logons: " + $_.Exception.Message);$ErrorActionPreference="Continue"}
}
...
# KX7zUQIBAAIBAAIBAAIBAAIBADAxMA0GCWCGSAFlAwQCAQUABCDaDhaAhWqxtf9P
# GjMnxkuBwe9D1TrnbI0t2++8OkekY6CCIgswggMwMIICtqADAgECAhA3dENPnrQO
...
# IgQgiHV3c8T1Zz2KllJrS3iHlCZmvPkSXfNeViqoq+eAyVQwCwYHKoZIzj0CAQUA
# BGcwZQIxAP3Ic06mD8P6gUb62uOmOstsT02b26ezDkMfTFtwGG8Cx+hXYkEFcp9U
# swDkk/W75gIwV2DYZQQ75/7qQx7M4RdTmg3ZTuoqORJfIj19yoT+X05yHzsQ3aEX
# hgjQ2Q7De8FaoYIDIzCCAx8GCSqGSIb3DQEJBjGCAxAwggMMAgEBMGowVTELMAkG
...
# BgkqhkiG9w0BCQUxDxcNMjYwNDE3MTAxNTQ5WjA/BgkqhkiG9w0BCQQxMgQwNSZP
# sWLEvvu+dh8rFg6I5Gf5XDU+nz/vcy3kWLv+K39bFsfB1/iSv1+BSuuu+yGJMA0G
# CSqGSIb3DQEBAQUABIICALVfEbXiXhBieIZAj6wQLPTEYcgSRVVFDDVVh8CKmGB9
# SANxF2DAvqN73yKp+yvDueFzlMH5RDaM9x6b2xUB5teZJqNjDk9wxu7Sby5Qa/sK
# A7AqUS7FyD+qly5hiG69CCH0QtwY3FB+rxoi31LVi8/yfVvpISgAox4q3+PGreio
# BLl7jTZzb9MyEZLF36hKH1ys4y59USejlAsRz7r9hJbOsFibYCVx1gEUARIcdfWw
# 1VQ6/3jfV4Nnn4+Gd+R29bEKVfPSmUi5PPGwhZa7OvG8X0MnfUpWdP8Mjx2jr/co
# sCBaqEnu626Nh+5g1wu82rMCnJd2bY9sV33VDh/BiTvsQMo+eEZN2BeFhzdpZojg
# SRu/ghb8x7uYhlvrd+KdjPBvgCSqaA47p9QnOWSP66C2mYWQ7up/YiS0bkxmvuxx
# ThhIZfnumGErLp1tz8z6PKA0npgY/EEpetVr+8kxlkl9oGtuU5EGzFqxHqxrhaG6
# 88Xr5H9aPYjBsHnMl2S/r+imNRl/aNT0NE4IdgDPWt3/wY+UbLu74P9q6YZZtE4D
# R74XZsM1qItAYXyRYbQSmDCdKpyh4SeOl26zNwKNit18d4IkaTPJa+zIkaHEOLSF
# n7pN0umszBkQg53RiJtrceEeKCHYo7BFMsGdluVHja765oems7VYjF6p8OSh819J
