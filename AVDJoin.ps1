...
$WVDdir=$env:SystemDrive + "\JoinWVD"
$Logfile = $WVDdir + "\JoinWVD.log"
...
if ($WVDToken)
	...
	logwrite ('WVD Token to join WVD Hostpool: ' + $WVDToken)
		...
		new-item -Path $WVDdir -ItemType Directory -ErrorAction SilentlyContinue
		Invoke-WebRequest -Uri $URI -OutFile $WVDdir\RDAgent.msi -UseBasicParsing
		Start-Process msiexec.exe -Wait -ArgumentList "/I $WVDdir\RDAgent.msi REGISTRATIONTOKEN=$WVDToken /qb"
		
		### Install RDBroker
		logwrite ('Install Remote Desktop Agent Boot Loader')
		$URI="https://query.prod.cms.rt.microsoft.com/cms/api/am/binary/RWrxrH"
		new-item -Path $WVDdir -ItemType Directory -ErrorAction SilentlyContinue
		Invoke-WebRequest -Uri $URI -OutFile $WVDdir\RDBoot.msi -UseBasicParsing
		Start-Process msiexec.exe -Wait -ArgumentList "/I $WVDdir\RDBoot.msi /qb"
