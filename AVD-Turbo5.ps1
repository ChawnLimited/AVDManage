...
Function CheckDomain
{
	try {
		...
		exit 401}
			...
			$actions = New-ScheduledTaskAction -Execute "powerShell.exe" -Argument "-NoProfile -WorkingDirectory $env:SystemRoot\Temp -Command  {Do {start-sleep -seconds 59;$i=$i+1} until (($proc=Start-Process dsregcmd -ArgumentList '/Join /debug' -Passthru -Wait -RedirectStandardOutput dsregcmd.log).ExitCode -eq 0 -or $i -eq 32)}" -WorkingDirectory "C:\Windows\System32"
			$at.actions=$actions
