...
%{
	try {
		$i=0
		Do{$i++; $Proc=Start-Process -FilePath dsregcmd.exe -ArgumentList '$(Arg0) $(Arg1) $(Arg2) /Debug' -RedirectStandardOutput AVD-EntraJoin.log -Wait -Passthru; LogWrite("Attempt:" + $i + " - DSRegCmd completed with exit code: " + $Proc.exitcode);CheckEntraID} until ($IsEntraJoined -eq "YES" -or $i -eq 20;Start-sleep -seconds 90)
