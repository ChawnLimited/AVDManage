...
# Wait for the SXS Network Agent and Geneva Agent to install
	try{		    
			...
			$i=0
			do {start-sleep -Seconds 2;$i++;} until((($SXS=get-package -name "*SXS*Network*" -ErrorAction SilentlyContinue).Status -eq 'Installed') -and (($Geneva=get-package -name "*Geneva*" -ErrorAction SilentlyContinue).Status -eq 'Installed') -or $i -eq 60)
		}
